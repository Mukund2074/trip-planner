...
export const ExpensesPage: React.FC = () => {
  ...
  const [splitAllMembers, setSplitAllMembers] = useState<boolean>(true);
  ...
  const handleOpenEditExpense = (expenseEntry: Expense) => {
    ...
    const splitIds = (expenseEntry.splitAmongMemberIds || []).map((participant) =>
      typeof participant === "object" ? participant._id : participant
    );

    if (splitIds.length === 0 || splitIds.length === memberList.length) {
      setSplitAllMembers(true);
      setSplitAmongMemberIds(memberList.map((member) => member._id));
    } else {
      setSplitAllMembers(false);
  ...
  const handleSaveExpense = async (eventObject: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        paidByMemberId,
        splitAmongMemberIds: splitAllMembers
          ? memberList.map((member) => member._id)
          : splitAmongMemberIds,
  ...
  const toggleSplitMember = (memberId: string) => {
    if (splitAmongMemberIds.includes(memberId)) {
      setSplitAmongMemberIds(
        splitAmongMemberIds.filter((existingId) => existingId !== memberId)
      );
    } else {
      setSplitAmongMemberIds([...splitAmongMemberIds, memberId]);
    }
  };

  const filteredExpenses = expenseList.filter((expenseItem) => {
    if (selectedCategory !== "All" && expenseItem.category !== selectedCategory) {
      return false;
    }
    return true;
  });
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-1">
            Track shared costs. Payer and trip organizers can confirm when individual members have paid their share.
          </p>
        </div>
      ...
      {!isMemberLoggedIn && (
        <div className="p-3.5 rounded-2xl bg-amber-50 border border-amber-200/80 flex items-center justify-between gap-3 text-xs text-amber-900">
          <div className="flex items-center gap-2">
            ...
            <span>
              <strong>View-only Mode:</strong> You can view all trip expenses and individual settlement statuses. Log in as a member to record expenses.
            </span>
          </div>
          <button
            onClick={openLoginModal}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="rounded-3xl p-6 bg-gradient-to-br from-slate-900 to-slate-800 text-white shadow-lg shadow-slate-900/10">
          ...
          <div className="text-3xl sm:text-4xl font-extrabold">
            ₹{summaryData ? summaryData.grandTotal.toLocaleString("en-IN") : "0"}
          </div>
        ...
        <div className="rounded-3xl p-6 bg-white border border-slate-200/80 shadow-sm flex flex-col justify-between">
          ...
          <div className="text-2xl sm:text-3xl font-extrabold text-emerald-700 my-1">
            ₹{summaryData ? summaryData.settledTotal.toLocaleString("en-IN") : "0"}
          </div>
        ...
        <div className="rounded-3xl p-6 bg-white border border-slate-200/80 shadow-sm flex flex-col justify-between">
          ...
          <div className="text-2xl sm:text-3xl font-extrabold text-amber-700 my-1">
            ₹{summaryData ? summaryData.pendingTotal.toLocaleString("en-IN") : "0"}
          </div>
          <p className="text-xs text-slate-500">
      ...
      {filteredExpenses.length === 0 ? (
        <div className="p-12 text-center bg-white rounded-3xl border border-slate-200 space-y-3 shadow-sm">
          ...
          <p className="text-xs text-slate-500 max-w-sm mx-auto">
            Record fuel, hotel deposits, meals, tickets, or other shared costs.
          </p>
          {isMemberLoggedIn ? (
            <button
              onClick={handleOpenAddExpense}
        ...
        <div className="space-y-4">
          <p className="text-xs text-slate-400 px-1">
            💡 The member who paid for an expense (or the trip organizer) can confirm individual repayments below. Other members cannot settle themselves.
          </p>
          ...
          {filteredExpenses.map((expenseEntry) => {
            ...
            const splitMembersList = (expenseEntry.splitAmongMemberIds || []).map(
              (participantItem) => {
            ...
            const splitCount = splitMembersList.length > 0 ? splitMembersList.length : 1;
              ...
              >
                ...
                <div className="pt-3 border-t border-slate-100">
                  <div className="text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-2 flex items-center justify-between">
                    <span>
                      Split Among {splitMembersList.length} Members (₹{shareAmount}/person):
                    </span>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                    {splitMembersList.map((participant) => {
                      ...
                      const hasPaid = isPayer || settledIdsList.includes(participant._id);
                          ...
                          <div className="flex items-center gap-2 min-w-0">
                            ...
                            <div className="min-w-0">
                              <div className="text-xs font-bold truncate">
                                {participant.name}{" "}
                                {currentMember?._id === participant._id ? "(You)" : ""}
                              </div>
                          ...
                          <div className="shrink-0">
                            {isPayer ? (
                              ...
                              <button
                                ...
                                className={`px-2.5 py-1 rounded-xl text-[11px] font-bold transition flex items-center gap-1 shadow-xs ${
                                  hasPaid
          ...
          >
                        ...
                        {(editingExpense.splitAmongMemberIds || []).map(
                          (participantItem) => {
                            ...
                            const participantMember =
                              typeof participantItem === "object"
                                ? participantItem
                                : memberList.find((m) => m._id === participantId) || {
                                    _id: participantId,
                                    name: "Member",
                                  };
                            ...
                            ).map((sc) => (typeof sc === "object" ? sc._id : sc));
            ...
            <form onSubmit={handleSaveExpense} className="space-y-3.5 text-sm">
              <div>
                ...
                <input
                  type="text"
                  placeholder="e.g. Highway Fuel & Toll / Theme Park Tickets / Resort Stay"
                  value={titleInput}
                  onChange={(changeEvent) => setTitleInput(changeEvent.target.value)}
                  required
                  disabled={!isMemberLoggedIn}
                  className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white outline-none disabled:bg-slate-100 disabled:text-slate-700"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    value={amountInput}
                    onChange={(changeEvent) => setAmountInput(changeEvent.target.value)}
                    required
                    disabled={!isMemberLoggedIn}
                    className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white outline-none font-bold disabled:bg-slate-100 disabled:text-slate-700"
                  />
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(changeEvent) =>
                      setCategoryInput(changeEvent.target.value as ExpenseCategory)
                    }
                    disabled={!isMemberLoggedIn}
                    className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white outline-none font-semibold disabled:bg-slate-100 disabled:text-slate-700"
                  >
                    <option value="Food">Food & Dining</option>
                    <option value="Travel">Travel & Fuel</option>
                    <option value="Stay">Hotel / Stay</option>
                    <option value="Tickets">Entry Tickets</option>
                    <option value="Activities">Activities / Rides</option>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(changeEvent) => setPaidByMemberId(changeEvent.target.value)}
                  disabled={!isMemberLoggedIn}
                  className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white outline-none font-semibold disabled:bg-slate-100 disabled:text-slate-700"
                >
              ...
              <div className="space-y-2 pt-1">
                <div className="flex items-center justify-between">
                  ...
                  {isMemberLoggedIn && (
                    <button
                      type="button"
                      onClick={() => setSplitAllMembers(!splitAllMembers)}
                      className="text-xs text-emerald-600 font-bold"
                    >
                      {splitAllMembers ? "All Members" : "Custom Selection"}
                    </button>
                  )}
                </div>

                {!splitAllMembers && (
                  <div className="p-3 rounded-2xl bg-slate-50 border border-slate-200 space-y-1.5 max-h-36 overflow-y-auto">
                    {memberList.map((member) => {
                      const isSelected = splitAmongMemberIds.includes(member._id);
                      return (
                        <div
                          key={member._id}
                          onClick={() => isMemberLoggedIn && toggleSplitMember(member._id)}
                          className="flex items-center gap-2 cursor-pointer text-xs font-medium text-slate-700 select-none"
                        >
                          <input
                            type="checkbox"
                            checked={isSelected}
              ...
              <div>
                ...
                <textarea
                  ...
                  value={notesInput}
                  onChange={(changeEvent) => setNotesInput(changeEvent.target.value)}
                  rows={2}
                  disabled={!isMemberLoggedIn}
                  className="w-full px-3.5 py-2 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white outline-none disabled:bg-slate-100 disabled:text-slate-700"
                />
              </div>
              ...
              <div className="flex gap-2 pt-2">
                ...
                {isMemberLoggedIn ? (
                  ...
                  <button
                    ...
                    className="w-1/2 py-2.5 rounded-xl bg-slate-900 text-white font-semibold text-xs shadow-md flex items-center justify-center gap-1.5"
                  >
